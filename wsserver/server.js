        ...
        wss.on('connection', (socket, req) => {
            ...
            socket.on('message', packet => {
                ...
                function exportModel() {
                    ...
                    fs.writeFile(`${config.model_dir}/${model}.model.json`, JSON.stringify(convert.convertDataValues(tempBlocks[commandSender])), function(err) {
                        if(err) {
                            sendCMD(`tellraw \"${commandSender}\" {"rawtext":[{"text":"An error occurred try to save the model: ${err}"}]}`);
                        } else {
                            sendCMD(`tellraw \"${commandSender}\" {"rawtext":[{"text":"Exported to ${config.model_dir}/${model}.model.json"}]}`);
                        }
                ...
                function importModel() {
                    ...
                    if(fs.existsSync(`${config.model_dir}.${modelName}.model.json`)) { // If models exists, load and place model
                        let modelJSON = JSON.parse(fs.readFileSync(`${config.model_dir}/${modelName}.model.json`), 'utf8');
